...
class TTSLSTM2V2(nn.Module):
    """One-step autoregressive TTS LSTM2 model.

    The three LSTM branches match TTSLSTM2. The head predicts only the next
    rental/return counts and receives target-time plus future weather features.
    """
    ...
    def __init__(
        ...
        dropout: float = 0.1,
        future_weather_dim: int = 4,
    ) -> None:
        super().__init__()
        if input_dim != 7:
            raise ValueError(f"tts_lstm2_v2 requires input_dim=7, got {input_dim}.")
        if target_time_dim != 8:
            raise ValueError(f"tts_lstm2_v2 requires target_time_dim=8, got {target_time_dim}.")
        if output_dim != 2:
            raise ValueError(f"tts_lstm2_v2 requires output_dim=2, got {output_dim}.")
        if future_weather_dim != 4:
            raise ValueError(f"tts_lstm2_v2 requires future_weather_dim=4, got {future_weather_dim}.")

        self.input_dim = int(input_dim)
        self.target_time_dim = int(target_time_dim)
        self.static_numeric_dim = int(static_numeric_dim)
        self.output_dim = int(output_dim)
        self.future_weather_dim = int(future_weather_dim)
    ...
    def forward(
        ...
        if target_time_features is None:
            raise ValueError("target_time_features is required for tts_lstm2_v2.")
        ...
        if future_weather_features is None:
            raise ValueError("future_weather_features is required for tts_lstm2_v2.")
        if future_weather_features.ndim != 2:
            raise ValueError(
                f"future_weather_features must have rank 2, got shape {tuple(future_weather_features.shape)}."
            )
        if future_weather_features.shape[-1] != self.future_weather_dim:
            raise ValueError(
                f"future_weather_features last dimension must be {self.future_weather_dim}, "
                f"got {future_weather_features.shape[-1]}."
            )
        ...
        if output.shape != (batch_size, self.output_dim):
            raise ValueError(
                f"tts_lstm2_v2 head output must have shape ({batch_size}, {self.output_dim}), "
