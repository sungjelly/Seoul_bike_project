...
class SeoulBikeLSTMDataset(Dataset):
    ...
    def _validate(self) -> None:
        ...
        if self.dynamic_features.shape[-1] != 7:
            raise ValueError(f"dynamic_features.npy last dimension must be 7, got {self.dynamic_features.shape[-1]}.")
        ...
        if self.targets.ndim != 4:
            raise ValueError("targets.npy must have shape (T, S, 8, 2).")
        T, S, _ = self.dynamic_features.shape
        if self.target_time_features.shape[0] != T:
            raise ValueError("target_time_features.npy first dimension must match dynamic_features.npy.")
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        item = {
            ...
            "target_time_features": self.torch.as_tensor(np.array(target_time_np, copy=True), dtype=self.torch.float32),
...
class FastLSTMBatchBuilder:
    ...
    def _build_batch(self, batch_ids: np.ndarray) -> dict:
        ...
        batch = {
            ...
            "target_time_features": self._to_tensor(target_time_np, self.torch.float32),
...
LSTM2Dataset = SeoulBikeLSTMDataset
LSTM2BatchBuilder = FastLSTMBatchBuilder


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Smoke-check an LSTM2 dataset batch.")
    parser.add_argument("--data-dir", type=Path, default=Path("data/lstm2_processed/tts_lstm2"))
...
def main() -> None:
    ...
    feature_config_path = args.data_dir / "feature_config.json"
    ...
    print(f"dynamic_feature_columns={feature_config['dynamic_feature_columns']}")
    print(f"target_time_feature_columns={feature_config['target_time_feature_columns']}")
